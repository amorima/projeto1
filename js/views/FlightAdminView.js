...
function loadDestinations() {
  ...
  if (fromSelect && toSelect) {
    ...
    const sortedDestinations = Array.from(destinationMap.values()).sort(
      (a, b) => a.cidade.localeCompare(b.cidade)
    );
  ...
  try {
    ...
    if (flightTableConfig.sortColumn) {
      flights.sort((a, b) => {
        ...
        return flightTableConfig.sortDirection === "asc"
          ? comparison
          : -comparison;
      });
    }
    ...
    const startIndex =
      (flightTableConfig.currentPage - 1) * flightTableConfig.rowsPerPage;
    ...
    tableBody.innerHTML = paginatedFlights
      .map(
        (flight) => `
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-150">
                <td class="px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">${escapeHtml(
                  flight.numeroVoo
                )}</td>
                ...
                <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-300 font-medium">€${parseFloat(
                  flight.custo || 0
                ).toFixed(2)}</td>
                <td class="px-6 py-4 text-sm text-right">
                    <div class="flex items-center justify-end gap-2">
                        <button onclick="editFlight('${
                          flight.numeroVoo
                        }')" class="p-1.5 text-primary hover:text-primary-dark hover:bg-gray-100 dark:hover:bg-gray-600 rounded-lg transition-colors duration-150" title="Editar voo">
          ...
          flight.numeroVoo
        )}')" class="p-1.5 text-red-600 hover:text-red-700 hover:bg-gray-100 dark:hover:bg-gray-600 rounded-lg transition-colors duration-150" title="Eliminar voo">
...
function getSortValue(flight, column) {
  switch (column) {
    ...
    case "leaves":
      return flight.partida || "";
    case "direct":
      return flight.direto === "S" ? "Sim" : "Não";
    case "custo":
      return (parseFloat(flight.custo) || 0).toString().padStart(10, "0");
    default:
      return "";
  }
}
...
function sortTableBy(column) {
  if (flightTableConfig.sortColumn === column) {
    flightTableConfig.sortDirection =
      flightTableConfig.sortDirection === "asc" ? "desc" : "asc";
  } else {
...
function updatePaginationControls(totalPages, currentPage) {
  ...
  if (totalPages <= 1) {
    paginationContainer.innerHTML =
      '<div class="text-sm text-gray-600 dark:text-gray-400">Total de resultados: 0</div>';
    return;
  }
  const startRecord = (currentPage - 1) * flightTableConfig.rowsPerPage + 1;
  const endRecord = Math.min(
    currentPage * flightTableConfig.rowsPerPage,
    totalPages * flightTableConfig.rowsPerPage
  );
...
function generatePageNumbers(totalPages, currentPage) {
  ...
  for (let i = startPage; i <= endPage; i++) {
    ...
    pages += `
            <button onclick="goToPage(${i})" class="px-3 py-2 text-sm font-medium ${
      isActive
...
function createFlight() {
  try {
    ...
    const flightData = {
      ...
      chegada: formatDateTime(formData.get("leaves")), // Por simplicidade, usa a mesma data que partida
      ...
      segmentos: [
        {
          ...
          companhia: formData.get("company"),
          partida: formatDateTime(formData.get("leaves")),
          chegada: formatDateTime(formData.get("leaves")),
        },
      ],
...
function editFlight(numeroVoo) {
  try {
    ...
    if (createBtn) {
      createBtn.innerHTML =
        '<span class="material-symbols-outlined text-lg">save</span><span>Guardar</span>';
    }
...
function updateFlight() {
  try {
    ...
    const flightData = {
      ...
      companhia: formData.get("company"),
      partida: formatDateTime(formData.get("leaves")),
      chegada: formatDateTime(formData.get("leaves")), // Keep existing or use partida
      direto: formData.get("direct") === "Sim",
      custo: parseFloat(formData.get("custo")) || 0,
      imagem: imagemUrl,
      ...
      pointsAR: Math.floor(parseFloat(formData.get("custo")) / 10) || 0,
    };
    // Validation
    if (
      !flightData.numeroVoo ||
    ...
    if (!existingFlight) {
      showToast("Voo não encontrado.", "error");
      return;
    } // Create updated flight object preserving existing properties and updating with new structure
    const updatedFlight = {
      ...existingFlight,
      ...
      chegada: flightData.chegada,
      direto: flightData.direto,
      custo: flightData.custo,
      imagem: flightData.imagem,
      turismo: existingFlight.turismo || flightData.turismo,
      segmentos: flightData.segmentos,
      pointsAR: flightData.pointsAR,
    };
...
function deleteFlight(numeroVoo, flightName) {
  showConfirm(
    `Tem a certeza que pretende eliminar o voo "${numeroVoo}"? Esta ação não pode ser desfeita.`
  ).then((confirmed) => {
    if (confirmed) {
      try {
...
function closeModalFlight() {
  ...
  if (createBtn) {
    createBtn.innerHTML =
      '<span class="material-symbols-outlined text-lg">add</span><span>Adicionar</span>';
  }
}
